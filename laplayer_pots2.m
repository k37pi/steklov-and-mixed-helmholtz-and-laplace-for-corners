...
% there is a negative sign here, but we just multiply 
% B with -1 at the end
...
% et = 0;
if sm > N
M2(isinf(M2)) = diag(dp1norm.*log(dp1norm)/pi)+etm;
% diag(dp1norm.*(mc-log(mu/2*dp1norm)/pi));
else
M2(isnan(M2)) = diag(dp1norm.*log(dp1norm)/pi)+etm;
% diag(dp1norm.*(mc-log(mu/2*dp1norm)/pi))+etm;    
end    
%-------------------------------------------
...
%% Matrices ------------------------------
A = (pi/N1*L2_tp).*dTau; % The second term is trapezoidal rule
